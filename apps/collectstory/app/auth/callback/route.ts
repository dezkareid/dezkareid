...
export async function GET(request: NextRequest) {
  ...
  const next = nextParameter && nextParameter.startsWith('/') && !nextParameter.startsWith('//') ? nextParameter : undefined;
  ...
  if (code) {
    ...
    if (!error && user) {
      ...
      if (next === undefined) {
        ...
        const destination = profile?.username ? `/${profile.username}` : '/profile/edit';
