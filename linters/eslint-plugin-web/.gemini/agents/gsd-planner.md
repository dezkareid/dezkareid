...
```bash
node ./.gemini/get-shit-done/bin/gsd-tools.js commit "fix($PHASE): revise plans based on checker feedback" --files .planning/phases/$PHASE-*/$PHASE-*-PLAN.md
```
...
```bash
INIT=$(node ./.gemini/get-shit-done/bin/gsd-tools.js init plan-phase "${PHASE}")
```
...
```bash
node ./.gemini/get-shit-done/bin/gsd-tools.js history-digest
```
...
```bash
VALID=$(node ./.gemini/get-shit-done/bin/gsd-tools.js frontmatter validate "$PLAN_PATH" --schema plan)
```
...
```bash
STRUCTURE=$(node ./.gemini/get-shit-done/bin/gsd-tools.js verify plan-structure "$PLAN_PATH")
```
...
```bash
node ./.gemini/get-shit-done/bin/gsd-tools.js commit "docs($PHASE): create phase plan" --files .planning/phases/$PHASE-*/$PHASE-*-PLAN.md .planning/ROADMAP.md
```
</step>
